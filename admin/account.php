...
// Variable to store current admin username
$admin_username  = $_SESSION['username'];

// Sql query to be executed for fetching data of user and it will be used or displayed further
$sql = "Select * from `libraryadmins` where username='$admin_username'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    if (isset( $_POST['editAccountDetails'])){
        /*** Update Admin Account Details Record ***/
        // Variables to be inserted into the table       
        $username = $_POST["usernameEdit"];
        $emailid  = $_POST["emailidEdit"];

        // Sql query to be executed
        $sql = "UPDATE `libraryadmins` SET `username` = '$username', `emailid` = '$emailid' WHERE `libraryadmins`.`username` = '$admin_username'";
        $result = mysqli_query($conn, $sql);       

        // Update Admin Account Details to the libraryadmins table in library database
        if($result){
            $updateAccountDetails = true;
            // Set updated username to username session variable
            $_SESSION['username'] = $username;
            // Reinitialize admin username variable with updated username session variable
            $admin_username  = $_SESSION['username'];
        ...
        if(password_verify($current_password, $row['password'])){
            if(($new_password == $confirm_password)){
                ...
                // Sql query to be executed
                // $admin_username variable is initialized with $_SESSION['username'] or current admin username in above portion of code
                $sql = "UPDATE `libraryadmins` SET `password` = '$hash' WHERE `libraryadmins`.`username` = '$admin_username'";
                $result = mysqli_query($conn, $sql);
                // Update Admin Account Password to the libraryadmins table in library database
                if ($result){
        ...
        <div class="account_container mx-auto shadow-lg bg-white rounded p-4">
            ...
            <div class="d-flex g-0">
                <div class="w-50 px-4 pt-1 pb-4 mx-auto">
                    ...
                    <form action="/publiclibrary/admin/account.php" method="post">
